"""Deployment-time hypothesis test: attitude tilt limiter via SO(3) controller.

Sets env.attitude_ctrl.max_tilt_rad to cap how far the controller tries to tilt,
regardless of policy command. Policy output is NOT touched — only the controller's
desired thrust direction (des_z_world) is cone-projected to within max_tilt of
+z_world. This preserves the policy's learned dynamics (its commands still go
through unchanged) while preventing catastrophic attitude divergence.

Unlike eval_mission_clipped.py (body-frame action norm clip → 32-49% catastrophic),
this clamps at the controller-target level, where attitude saturation already
implicitly happens for the baseline policy.

Run:
    conda run -n isaaclab311 python scripts/eval_mission_tilt.py --max_tilt 45
"""
...
parser.add_argument("--max_tilt", type=float, default=45.0,
                    help="Max tilt angle (degrees) for SO(3) cone projection. 0 or negative = disabled.")
parser.add_argument("--missions", type=int, default=300)
...
DOCK_TIMEOUT = 1800
TARGET_MISSIONS = args.missions
MAX_TILT_DEG = args.max_tilt
MAX_TILT_RAD = math.radians(MAX_TILT_DEG) if MAX_TILT_DEG > 0 else None

if MAX_TILT_RAD is not None:
    max_lateral_g = math.tan(MAX_TILT_RAD)
    print(f"\n>>> Tilt limiter ACTIVE: max_tilt = {MAX_TILT_DEG:.1f}°")
    print(f">>> Max sustained lateral accel (world) = g*tan(max_tilt) = {max_lateral_g*9.81:.2f} m/s² ({max_lateral_g:.2f}g)")
else:
    print(f"\n>>> Tilt limiter DISABLED (baseline)")
print(f">>> Target missions: {TARGET_MISSIONS}\n")
...
# ============ TILT LIMITER ACTIVATION ============
# Set on the attitude controller so SO(3) tracker projects des_z_world to safety cone.
if MAX_TILT_RAD is not None:
    env.attitude_ctrl.max_tilt_rad = MAX_TILT_RAD
    print(f">>> attitude_ctrl.max_tilt_rad set to {MAX_TILT_RAD:.4f} rad ({MAX_TILT_DEG:.1f}°)")
...
for step in range(MAX_STEPS * 10):
    if results["total"] + results["full_success"] >= TARGET_MISSIONS: break
    ...
    if a_mask.any():
        ...
        # APPROACH is unloaded — do NOT clip (this is the baseline flight policy)
        ...
        for i, eid in enumerate(a_ids.tolist()):
            ...
            if last_wp or timeout:
                ...
                if stable:
                    ...
                    else: phase[eid] = PHASE_DONE; record_fail(eid, "approach_timeout_too_far")
    ...
    if d_mask.any():
        ...
        for i, eid in enumerate(d_ids.tolist()):
            ...
            if box_z_local < 0.30 and contain < 150:
                phase[eid] = PHASE_DONE; record_fail(eid, "box_fell_during_dock"); continue
            if contain >= 325:
            ...
            if dock_steps[eid].item() > DOCK_TIMEOUT:
                phase[eid] = PHASE_DONE; record_fail(eid, "dock_timeout")
    ...
    if c_mask.any():
        ...
        for i, eid in enumerate(c_ids.tolist()):
            ...
            if cs > 60 and drone_z_local < 0.30:
                phase[eid] = PHASE_DONE; record_fail(eid, "climb_failed"); continue
            if cur_z > 1.0:
                ...
                phase[eid] = PHASE_DONE; record_fail(eid, "climb_timeout")
            elif cur_z < 0.05:
                phase[eid] = PHASE_DONE; record_fail(eid, "climb_crashed")
    ...
    if v_mask.any():
        ...
        # Tilt limiter applies inside attitude_ctrl.compute(); action passes through unchanged.
        ...
        for i, eid in enumerate(v_ids.tolist()):
            ...
            if box_z_l < 0.30 or box_dist > 0.50:
                phase[eid] = PHASE_DONE; record_fail(eid, "box_dropped_delivery"); continue
            if dist_wp[i].item() < WP_REACH_DIST:
                wi = wp_idx[eid].item() + 1
                if wi < n_delivery_wps[eid].item():
                    wp_idx[eid] = wi; current_goal[eid] = delivery_wps[eid, wi]
                else:
                    phase[eid] = PHASE_ARRIVED; results["full_success"] += 1
    ...
    if step % 3000 == 0:
        ...
        print(f"  [{step/150:5.0f}s] missions={results['total']+results['full_success']}/{TARGET_MISSIONS} success={results['full_success']} ({100*results['full_success']/n:.0f}%)")

n = results["total"] + results["full_success"]
_label = f"max_tilt={MAX_TILT_DEG:.1f}°" if MAX_TILT_RAD is not None else "DISABLED (baseline)"
print(f"\n{'='*60}\n  RESULTS (tilt_limiter: {_label})\n{'='*60}")
print(f"  Total missions:    {n}")
print(f"  Full success:      {results['full_success']}/{n} ({100*results['full_success']/max(n,1):.1f}%)")
for reason, count in sorted(fail_reasons.items(), key=lambda x: -x[1]):
    print(f"  {reason:30s} {count:>3}/{n} ({100*count/max(n,1):.1f}%)")
print(f"{'='*60}")
