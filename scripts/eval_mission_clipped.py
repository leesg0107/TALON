"""Zero-cost hypothesis test: clip policy action before passing to env.

Tests whether bounded action norm alone reduces delivery failures.
Same eval setup as eval_mission_headless.py except: every delivery-phase action
is clipped to ||action_xy|| <= ACTION_CLIP (in normalized [-1,1] space).

Try several ACTION_CLIP values sequentially to find sensitivity.
Run:
    conda run -n isaaclab311 python scripts/eval_mission_clipped.py
"""
...
parser.add_argument("--clip", type=float, default=1.0,
                    help="Max ||action_xy|| in normalized space ([-1,1]). 1.0 = nominal scaled, 0.5 = half, etc.")
...
ACTION_CLIP = args.clip

print(f"\n>>> Action clip test: ||action_xy|| <= {ACTION_CLIP} (normalized space)")
print(f">>> Equivalent physical limit: |a_xy| <= {ACTION_CLIP * 8.0} m/s² ({ACTION_CLIP*8.0/9.81:.2f}g)")
...
def clip_xy_norm(act, max_norm):
    """Clip ||action[:,:2]|| to max_norm (preserve direction). Leaves az and yaw unchanged."""
    xy = act[:, :2]
    norm = torch.norm(xy, dim=-1, keepdim=True).clamp(min=1e-6)
...
for step in range(MAX_STEPS * 10):
    ...
    if v_mask.any():
        ...
        # ============ HYPOTHESIS TEST: CLIP action xy norm ============
        act = clip_xy_norm(act, ACTION_CLIP)
...
print(f"\n{'='*60}\n  RESULTS (clip={ACTION_CLIP}, equiv |a_xy| <= {ACTION_CLIP*8.0:.1f} m/s²)\n{'='*60}")
