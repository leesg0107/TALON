"""Diagnostic for CLIMB failures and CLIMB→DELIVERY state propagation.

Answers two questions:
  Q1. Why does PID-controlled CLIMB fail (climb_failed in 10% of missions)?
      → Records per-step (z, vel_z, tilt, ang_vel_norm, box_z) during CLIMB.
  Q2. When CLIMB succeeds, does residual instability cause DELIVERY failure?
      → Snapshots full state at CLIMB→DELIVERY transition, correlates with outcome.

Outputs:
  logs/diagnose_climb_delivery_raw.npz
    climb_entry      — (N_missions, 18)  per-mission CLIMB entry features
    climb_traces     — list of (T, 6)   per-mission CLIMB phase trajectories
    delivery_entry   — (N_missions, 18)  per-mission DELIVERY entry features (NaN if climb failed)
    outcomes         — (N_missions,)     string array
    dr_samples       — (N_missions, 2)   (mass_scale, payload_mass)

Run:
    conda run -n isaaclab311 python scripts/diagnose_climb_delivery.py
"""
import sys, os, math
...
DOCK_TIMEOUT = 1800
MAX_CLIMB_STEPS = 2000        # cap per-episode CLIMB trace length
...
def snapshot_state(eid):
    """Capture full physical state at a phase boundary.
    Features (18): drone_pos(3 local), drone_vel(3), drone_tilt_deg, drone_ang_vel_b_norm,
                   box_pos(3 local), box_vel(3), box_offset_in_gripper_body(3),
                   contain_hold_count, drone_drone_dist_to_box.
    """
...
def per_step_climb_features(eid):
    """Per-step features during CLIMB (6-D).
    Features: t_steps, drone_z_local, drone_vel_z, drone_tilt_deg, ang_vel_norm, box_z_local
    """
    origin = env.scene.env_origins[eid]
    pos_w = env.robot.data.root_pos_w[eid]
    vel_w = env.robot.data.root_lin_vel_w[eid]
    ...
    return np.array([
        pos_w[2].item() - origin[2].item(),
        vel_w[2].item(),
        math.degrees(math.acos(max(-1.0, min(1.0, R[2, 2].item())))),
        torch.norm(ang_vel_b).item(),
        obj_pos[2].item() - origin[2].item(),
        torch.norm(pos_w - obj_pos).item(),
    ], dtype=np.float32)
...
# Per-env diagnostic buffers
climb_entry_data    = [None] * NUM_ENVS   # 18-D snapshot at DOCK→CLIMB
delivery_entry_data = [None] * NUM_ENVS   # 18-D snapshot at CLIMB→DELIVERY
climb_trace_buf     = [None] * NUM_ENVS   # (T, 6) accumulating
climb_trace_len     = [0]    * NUM_ENVS
dr_sample           = [None] * NUM_ENVS   # (mass_scale, payload_mass)

# Aggregated mission records
mission_records = []      # list of dict per completed mission
...
def finalize_mission(eid, outcome):
    ...
    # Trim climb trace
    trace = None
    if climb_trace_buf[eid] is not None and climb_trace_len[eid] > 0:
        trace = climb_trace_buf[eid][:climb_trace_len[eid]].copy()

    mission_records.append({
        "outcome": outcome,
        "climb_entry": climb_entry_data[eid],         # 18-D or None
        "delivery_entry": delivery_entry_data[eid],   # 18-D or None
        "climb_trace": trace,                          # (T, 6) or None
...
print(f"\n{'='*60}")
print(f"  CLIMB + DELIVERY PROPAGATION DIAGNOSTIC ({NUM_ENVS} envs, target={TARGET_MISSIONS})")
print(f"{'='*60}\n")
...
for step in range(MAX_STEPS * 10):
        ...
        for i, eid in enumerate(a_ids.tolist()):
            ...
            if last_wp or timeout:
                ...
                if stable:
                    ...
                    if xy < 2.0 and za > 0: phase[eid] = PHASE_DOCK; dock_steps[eid] = 0
        ...
        for i, eid in enumerate(d_ids.tolist()):
            ...
            if contain >= 325:
                ...
                if box_drone_dist < 0.30:
                    ...
                    # === CAPTURE: DOCK→CLIMB entry state ===
                    ...
                    climb_trace_buf[eid] = np.zeros((MAX_CLIMB_STEPS, 6), dtype=np.float32)
                    climb_trace_len[eid] = 0
    ...
    if c_mask.any():
        ...
        # === RECORD per-step CLIMB features ===
        for eid in c_ids.tolist():
            if climb_trace_buf[eid] is not None and climb_trace_len[eid] < MAX_CLIMB_STEPS:
                climb_trace_buf[eid][climb_trace_len[eid]] = per_step_climb_features(eid)
                climb_trace_len[eid] += 1
        for i, eid in enumerate(c_ids.tolist()):
            ...
            if cur_z > 1.0:
                # === CAPTURE: CLIMB→DELIVERY entry state ===
...
np.savez_compressed(
    ...
    climb_traces=np.array([r["climb_trace"] for r in mission_records], dtype=object),
    dr_samples=np.array([r["dr_sample"] for r in mission_records]),
)
print(f"\n  Saved: logs/diagnose_climb_delivery_raw.npz ({len(mission_records)} records)")
