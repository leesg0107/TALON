"""v3 diagnostic — what physically determines box_offset_y at bifurcation?

We previously found:
  - box_offset_y starts ≈ 0 for ALL groups at contain=100
  - It diverges at contain 230-280 (plate close)
  - Final value (-0.026 deep vs -0.015 shallow) determines downstream success
  - Raising dock altitude HURT — disproved "altitude origin" hypothesis

This v3 captures richer per-step features during the dock phase:
  - drone yaw (world frame)
  - plate joint angles (left, right)
  - box velocity (world frame)
  - drone XY in world frame
  - box XY in world frame
  - DR samples (mass_scale, payload_mass)

Goal: identify which factor PHYSICALLY causes the bifurcation between
deep (box wedged) and shallow (box centered) grasps.

Run:
    conda run -n isaaclab311 python scripts/diagnose_dock_mechanism.py
"""
import sys, os, math
...
# Find plate joint IDs
plate_joint_ids, _ = env.robot.find_joints("plate_joint_.*")
print(f"Plate joint IDs: {plate_joint_ids}")
...
def per_step_v3_features(eid):
    """V3 per-step DOCK features (14-D)."""
    ...
    plate_l = joint_pos[plate_joint_ids[0]].item()
    plate_r = joint_pos[plate_joint_ids[1]].item() if len(plate_joint_ids) > 1 else plate_l

    contain = env.contain_hold_count[eid].item()
    drone_z = pos_w[2].item() - origin[2].item()
    box_z = obj_pos[2].item() - origin[2].item()
    yaw = quat_to_yaw(quat_w[0])

    # Box offset in body frame
    gripper_offset_w = R @ torch.tensor([0., 0., -0.08], device=device)
    box_rel_w = obj_pos - (pos_w + gripper_offset_w)
    box_offset_b = R.T @ box_rel_w

    # Drone-box XY in WORLD frame
    drone_x_local = pos_w[0].item() - origin[0].item()
    drone_y_local = pos_w[1].item() - origin[1].item()
    box_x_local = obj_pos[0].item() - origin[0].item()
    box_y_local = obj_pos[1].item() - origin[1].item()

    return np.array([
        contain,                          # 0
        drone_z, box_z,                   # 1-2: vertical positions
        yaw,                              # 3: drone yaw (rad)
        plate_l, plate_r,                 # 4-5: plate joint angles
        box_offset_b[0].item(),           # 6: box_offset_x (body, strut)
        box_offset_b[1].item(),           # 7: box_offset_y (body, plate)
        box_offset_b[2].item(),           # 8: box_offset_z (body, vertical)
        drone_x_local - box_x_local,      # 9: drone-box x in WORLD
        drone_y_local - box_y_local,      # 10: drone-box y in WORLD
        obj_vel[0].item(), obj_vel[1].item(),  # 11-12: box velocity world XY
        math.degrees(math.acos(max(-1.0, min(1.0, R[2, 2].item())))),  # 13: tilt deg
    ], dtype=np.float32)
...
final_box_offset_y = [None] * NUM_ENVS
final_outcome = [None] * NUM_ENVS
...
def setup_mission(eids):
    for eid in eids.tolist():
        ...
        final_box_offset_y[eid] = None
        final_outcome[eid] = None
...
def finalize_mission(eid, outcome):
    ...
    mission_records.append({
        ...
        "final_box_offset_y": final_box_offset_y[eid],
...
print(f"\n{'='*60}\n  V3 DOCK MECHANISM DIAGNOSTIC ({NUM_ENVS} envs, target={TARGET_MISSIONS})\n{'='*60}\n")
...
for step in range(MAX_STEPS * 10):
        ...
        for i, eid in enumerate(a_ids.tolist()):
            ...
            if last_wp or timeout:
                ...
                if stable:
                    ...
                    dock_trace_buf[eid] = np.zeros((MAX_DOCK_STEPS, 14), dtype=np.float32)
                    dock_trace_len[eid] = 0
                elif timeout:
                    if xy < 2.0 and za > 0:
                        phase[eid] = PHASE_DOCK; dock_steps[eid] = 0
    ...
    if d_mask.any():
        ...
        # Record full dock trajectory (from contain=50 onward = approach phase + plate close)
        for eid in d_ids.tolist():
            contain = env.contain_hold_count[eid].item()
            if contain >= 50 and dock_trace_buf[eid] is not None and dock_trace_len[eid] < MAX_DOCK_STEPS:
                dock_trace_buf[eid][dock_trace_len[eid]] = per_step_v3_features(eid)
...
np.savez_compressed(
    "logs/diagnose_dock_mechanism_raw.npz",
    ...
    final_box_offset_y=np.array([r["final_box_offset_y"] if r["final_box_offset_y"] is not None else np.nan for r in mission_records]),
    dr_samples=np.array([r["dr_sample"] for r in mission_records]),
)
print(f"\n  Saved: logs/diagnose_dock_mechanism_raw.npz ({len(mission_records)} records)")
